...
//========================================================================================
	...
	Flux<User> betterCallSaulAndJesseForBogusFlux(Flux<User> flux) {
		return flux.onErrorResume(e-> Flux.just(User.SAUL,User.JESSE)); //예외 발생 시 다른 flux형태로 return
	}
	...
	Flux<User> capitalizeMany(Flux<User> flux) {
		return flux.map(user -> {
			try {
				...
				throw Exceptions.propagate(e); // RuntimeException을 상속받지 않은 예외를 RuntimeException으로 바꾸어주는 함수
			}
		});
	}
